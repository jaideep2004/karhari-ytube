...
const ARTWORK_DIR = path.join(UPLOAD_DIR, 'artwork');
...
function escapeDrawtext(text: string): string { return text.replace(/\\/g, '\\\\').replace(/:/g, '\\:').replace(/'/g, "\\'").replace(/%/g, '\\%'); }
...
function presetColorHex(color?: string): string {
  return PRESET_COLOR_HEX[color || 'cyan'] || PRESET_COLOR_HEX.cyan;
}
...
async function getFfmpeg() {
  ...
  try {
    ...
    if (!ffmpegPath) {
      try { ffmpegPath = (await import('ffmpeg-static')).default as unknown as string; } catch {}
    }
    if (!ffprobePath) {
      try { ffprobePath = (await import('ffprobe-static')).path as unknown as string; } catch {}
      if (!ffprobePath) {
        try { ffprobePath = (await import('ffprobe-static')).default as unknown as string; } catch {}
      }
    }
...
export async function resolveMedia(
  ...
  if (filename.startsWith('http://') || filename.startsWith('https://')) {
    ...
    const tempPath = path.join(/* turbopackIgnore: true */ SOCIAL_VIDEO_DIR, `${uuidv4()}${ext}`);
...
export async function generateVideo(input: GenerateVideoInput): Promise<GenerateVideoResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // itself covers 1920x1080. Users are advised in the UI to upload landscape 1920x1080.
    ...
    const LOGO_PATH_PNG = path.join(process.cwd(), 'public', 'images', 'karhari-media-b1.png');
    const LOGO_PATH_FALLBACK = path.join(process.cwd(), 'public', 'karhari-media-b1.png');
    const LOGO_PATH_SVG = path.join(process.cwd(), 'public', 'file.svg');
    const LOGO_PATH = existsSync(LOGO_PATH_PNG) ? LOGO_PATH_PNG : (existsSync(LOGO_PATH_FALLBACK) ? LOGO_PATH_FALLBACK : LOGO_PATH_SVG);
    ...
    command
      ...
      .on('start', (cmdLine: string) => {
        console.log(`[ffmpeg] start: ${cmdLine.slice(0, 4000)}`);
        console.log(`[ffmpeg] filterComplex: ${filterComplex.slice(0, 4000)}`);
      })
      ...
      .on('stderr', (line: string) => {
        ...
        (globalThis as any).__ffmpegStderr = (((globalThis as any).__ffmpegStderr || '') + line + '\n');
        if (((globalThis as any).__ffmpegStderr as string).length > 8000) (globalThis as any).__ffmpegStderr = (globalThis as any).__ffmpegStderr.slice(-8000);
      })
      .on('error', (err: Error) => {
        clearTimeout(timeout);
        input.signal?.removeEventListener('abort', onAbort);
        cleanupTempFiles(...tempCleanup);
        const tail = ((globalThis as any).__ffmpegStderr || '').slice(-3000).replace(/\n/g, ' | ');
        const fcSnippet = (typeof filterComplex !== 'undefined' ? filterComplex : '').slice(0, 2500);
        console.error(`[ffmpeg:error] ${err.message} | tail: ${tail} | filterComplex: ${fcSnippet}`);
        reject(new Error(`Video generation failed: ${err.message} | filter: ${fcSnippet.slice(0,800)} | stderr: ${tail.slice(0,800)}`));
      })
      .on('end', async () => {
        clearTimeout(timeout);
        input.signal?.removeEventListener('abort', onAbort);
